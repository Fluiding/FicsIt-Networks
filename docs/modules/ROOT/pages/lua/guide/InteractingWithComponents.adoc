...
and then interact with the storage container.
...
It will convert the reference to a descriptive string for easy debugging.
...
The value that the proxy function returns is also referred to as
Object-, Component-, -Instance or -Reference. +
These are special values created by FIN and not native to Lua. +
...
If the proxy function cannot find a network component with the given id,
it will return `nil`.
...
Additionally, the proxy function can take a variable number of arguments
...
and they have functions to interact with the object. +
Also important to note is that each object has a "Type". You call the objects also "instances of a class".
A class is nothing more than an object that describes another object. +
If you e.g. have a storage container and use the proxy function to get a reference to it.
You reference an instance of type storage container
("storage container" is the type of the object, and the specific object with the ID you used in the proxy is an instance of it).
The class (aka. type) "storage container" now describes what attributes and functions a storage container has.
A constructor e.g. has a `standby` attribute that describes if the constructor is turned off or not,
...
A constructor e.g. shares the standby functionality with many other machines just like power generators,
...
The "constructor" class/type extends the "factory" class/type which adds e.g. standby functionality.
...
Instead, it's stored in the actual object you use, and that is why you have to pass it to the function as you use it.
====
...
* `hash` attribute and `getHash()` function +
  ...
  You want to use this when you e.g. use a object as table key. +
  ...
  Lua will view them as individual values. And that means it cannot be used as table key. +
...
* `getType()` function +
  This function allows you to get an object that represents the type (not an individual instance) of an object. +
  With it, you can e.g. dynamically discover attributes and functions, show their descriptions etc. +
...
e.g. an Item Type (Iron Plate), doesn't have instances, there are not many "Iron Plate Types",
...
This behavior is implemented by overwriting the index and new-index functions in the meta-table.

Referencing a component is a bit special. +
Such reference will have additionally two more properties you can work with. +
The additional read-only `id` field allows you to get the UUID of the component. +
The additional `nick` field allows you to read and write the component's nick.
...
We could separate the part where we get the item-count from the inventory and the part where we print to the console,

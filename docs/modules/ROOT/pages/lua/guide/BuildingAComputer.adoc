...
We will discuss these advanced features in separate sections.
...
to communicate and interact with each other. This system is also referred as "FicsIt-Kernel".
...
This section is like the cover of your real computer and houses components like the motherboard.
...
Some other parts are crucial to the computer's function.
...
Right now there is only the `Lua CPU` that allows for running Lua code in the computer.
...
The Lua code tab is split into four sections.
...
* On the bottom left you can find the power button and status LEDs. +
  ...
  The warning sign is unlit by default. If the computer crashes, it will light up.
  Its state will be reset with the next boot of the computer.
...
For this guide, you should first craft a single EEPROM and insert into the computer you just built.

...
print("Hello World")
----
just add it to the code window and hit the power button.

Everyone else who wants to start with something proper.
We first have to learn about something very important.
...
The first is the component id. This is a UUID that identifies a unique component and cannot be changed.
It should be unique in the universe (but real life won't allow that, for now we can view it as such).
An analogy would be an IP-Address or a MAC-Address. +
...
You can e.g. give a component the nick `test north 1`
...
But your program will have to figure out how to parse this data itself.

[WARNING]
====
In a component network there can be an infinite amount of network components,
but large networks are not recommended as they can slow the game down
...
network cards f.e. are connected to the network via an internal connection inside the computer case.
...
Once attached it will stay attached until you disassemble the whole machine/thing.
...
Other buildings like a storage container which doesn't have a power connector
...
It provides an input field to change the network component's nick, +
...
of one data storage item to another one. +
...
You can not copy the data from an EEPROM to a drive or vice versa,
